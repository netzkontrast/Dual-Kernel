...
```bash
# Resolve MNEMONIC_ROOT to the current workspace root
MNEMONIC_ROOT="${MNEMONIC_ROOT:-.}"

# For Dual-Kernel Kohärenz Protokoll, we use the local root namespace directories directly
# instead of an {org}/{project} hierarchy to stay consistent with the _episodic, _semantic, _procedural paradigm.
